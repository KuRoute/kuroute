...
func (s *PackageService) canUpdatePackageStatus(current, next domain.PackageStatus, role domain.UserRole) bool {
    ...
    case domain.PackageStatusSorted:
        // sorted -> assigned: staff_sortir via BatchAssignmentService (internal call)
        return next == domain.PackageStatusAssigned && role == domain.UserRoleStaffSortir
    ...
    default:
        // received -> sorted dan semua terminal state (delivered, failed)
        return false
    }
}
